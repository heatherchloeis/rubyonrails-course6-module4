class ThingTagsController < ApplicationController
  ...
  after_action :verify_policy_scoped, only: [:linkable_tag_things]

  def index 
    authorize @thing, :get_tags?
  	@thing_tags = @thing.thing_tags.with_keyword
  end
  def tag_things
  ...
  def linkable_tag_things
    authorize Thing, :get_linkable_tags?
